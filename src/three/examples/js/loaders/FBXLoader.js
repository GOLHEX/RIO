...
import Zlib from "../libs/inflate.min";;





const _FBXLoader =  ( function () {
	...
	FBXLoader.prototype = {
		...
		parse: function ( FBXBuffer, path ) {
			...
			//console.log(new FBXTreeParser( textureLoader ).parse( fbxTree ))

			//var ftp = new FBXTreeParser( textureLoader ).parse( fbxTree );
		...
		parse: function ( buffer ) {
			...
			while (wh) {
				...
				let node = this.parseNode( reader, version );
 				//console.log(allNodes.add( node.name, node ))
				//allNodes.add( node.name, node );

				 if ( node !== null ) {
				 	allNodes.add( node.name, node )
				 	console.log('Node: '+node.name+ ' added to allNode')
				 } else {
				 	console.log('Break allNode ')
				 	//break
				 }

				 if(allNodes.Definitions){
				 	wh = 0;
